...
if not text:
return set()
Copy# Convert to lowercase and split on non-alphanumeric characters
...
Copy    # Normalize to prevent division by zero
...
results = []
Copyasync def _embed(txt: str, idx: int):
    async with sem:
...
explanations = []
Copyasync def _get_explanation(row_idx: int):
...
if not topic:
return df
Copy# Check if topic appears in title or description
...
Copy# Create a visual representation with custom colors
colors = {
    "poor": "#CDCDCD",
    "low": "#ADD8E6",
    "medium": "#D4AC0D",
...
Copyst.subheader("Search Parameters")
...
CopyOptional search data columns are also supported.
""")
...
Copyif st.session_state['embed_status']:
...
if len(results) == 0:
    ...
    st.download_button("💾 Download Recommendations", csv_bytes, "conten
